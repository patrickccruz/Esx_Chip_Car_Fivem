...
RegisterNUICallback('save', function(data, cb)
    ESX.TriggerServerCallback('qb-tunerchip:server:HasChip', function(HasChip) 
        if HasChip then
            ...
            TriggerServerEvent('qb-tunerchip:server:TuneStatus', plate, true)
        end
        cb('ok')
    end)
end)
...
RegisterNetEvent('qb-tunerchip:client:TuneStatus', function()
    ...
    if vehModel ~= 0 then
        ESX.TriggerServerCallback('qb-tunerchip:server:GetStatus', function(status) 
            if status then
...
RegisterNUICallback('checkItem', function(data, cb)
    -- local retval = false
    -- ESX.TriggerServerCallback('QBCore:HasItem', function(result) 
    --     if result then
    --         retval = true
    --     end
        cb(true)
